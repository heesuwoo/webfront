...
function stringTest(){
    ...
    div1.innerHTML += "<hr>";
    // div1.innerHTML += "str1.charAt(0) : " + str1.charAt(0) + "<br>" // 'H'

    // for문을 이용해 str1에 저장된 문자 하나씩 모두 출력
...
function numberTest(){
    ...
    // Java에서는 0으로 나누기, 숫자가 아닌 값들의 연산을 예외 또는 컴파일에러로 막아두었지만
    // JS는 예외/컴파일에러가 없으므로 별도 처리 방법인 Infinity와 NaN이 존재한다.
    ...
    // 소수점 첫째자리에서 올림/내림/반올림/버림
    div2.innerHTML += "올림 -> Math.ceil(숫자) : " + Math.ceil(10.1) + "<br>";
    div2.innerHTML += "내림 -> Math.floor(숫자) : " + Math.floor(10.9) + "<br>";
    div2.innerHTML += "반올림 -> Math.round(숫자) : " + Math.round(10.4) + "<br>";
    div2.innerHTML += "반올림 -> Math.round(숫자) : " + Math.round(10.5) + "<br>";
    div2.innerHTML += "버림 -> Math.trunc(숫자) : " + Math.trunc(-10.5) + "<br>";
}
...
console.log(1==true);   //true ( c언어에서는 true == 1 )
...
console.log(1===1);    //true ( 값, 자료형 모두 같음)
