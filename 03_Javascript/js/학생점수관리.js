...
document.getElementById("input-student").addEventListener("click", function(){
    ...
    // 3) tr 태그 요소 생성
    const tr = document.createElement("tr");    // 아직 화면 배치 X
    ...
    for(let input of inputArr){
        ...
        // 8) input 태그의 값이 입력되지 않은 경우
        if(input.value.trim() == ""){
            // 문자열.trim() : 좌우 공백 제거
            
            // 9) 경고를 띄우고 미입력된 input 태그에 focus 옮기기
            alert("값을 모두 입력해주세요.");
            input.focus();
        ...
        // 5) 생성된 td 요소를 3)번에서 생성한 tr 요소에 자식으로 추가
        tr.appendChild(td);
    ...
    // 6) #view-student 테이블에 tr 요소를 자식으로 추가
    document.getElementById("view-student").appendChild(tr);
    
    // 추가2) stdList에 생성된 Student 객체 추가하기
    stdList.push(std);
});
...
document.getElementById("age-asc-btn").addEventListener("click", function(){
    ...
    // 배열명.slice([시작 인덱스, 길이]): 배열을 시작인덱스부터 길이만큼을 잘라내어 새로운 배열을 만듦
    //                                  원본 훼손없이 새로운 배열 생성(깊은 복사)
...
// 나이 내림차순 버튼이 클릭 되었을 때의 동작
document.getElementById("age-desc-btn").addEventListener("click", function(){
    
    const copyList = stdList.slice();   // stdList 깊은 복사
    copyList.sort(ageDesc);  // copyList 나이 내림차순 정렬
    createView(copyList);   // 화면 출력
});


// #view-student의 내용을 모두 삭제하고 전달받은 학생 리스트를 이용해서 화면을 만드는 함수
